...
class ProductManager {
    ...
    static idProducts //goblal variable initialized at 0 in DB
   ...
   // Agregar producto a DB
    async addProduct(obj) {
        try {            
            //Traigo el id de la DB y actualizo var global
    ...
    async getProductById(id) {
        try {            
            const producto = await this.products.find(e => e.id === id)
            if (producto) {
                return producto
            } else {
    ...
    async deleteProduct(id) {
        try {          
            let producto = await this.products.find(e => e.id === id)
            if (producto) {
    ...
    async updateProduct(id, obj) {
        try {
            ...
            let producto = await this.products.find(e => e.id === id)
            if (producto) {
                console.log("El producto a editar es " + producto.title)
            } else {
